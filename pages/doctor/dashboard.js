    ...
    useEffect(() => {
        async function getData(){
          const config={
            ...
            try{
                var {data} = await axios.get("/api/appointment/doctor",config);
                console.log(data);
                setAppn(data);
            }catch(error){
              console.log("here in error in doctor dashboard");
        ...
        <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50 py-10 px-4 flex flex-col items-center">
            <div className="w-full max-w-4xl">
                ...
                <div className="mb-8 text-center">
                    ...
                    <p className="text-gray-600 text-base">Overview of your appointments and schedules</p>
                </div>
                    ...
                    <div className="overflow-x-auto">
                        <table className="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody className="bg-white divide-y divide-gray-100">
                                {appn.length === 0 && (
                                    <tr>
                                        <td colSpan={4} className="text-center py-8 text-gray-400 text-lg">No appointments yet</td>
                                    </tr>
                                )}
                                {appn.map((person, i) => (
                                    <tr key={i} className="hover:bg-indigo-50 transition duration-200">
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div className="text-sm text-gray-700 font-medium">{moment(person.date).format('DD/MM/YYYY')}</div>
                                            </div>
                                        </td>
                                        <td className="px-6 py-4 whitespace-nowrap">
